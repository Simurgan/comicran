...
void get_executable_dependencies(const char *executable, FileCopy *file_copies, int *file_copy_count, int max_file_copies, char **directories, int *dir_count, int max_directories);
void write_output_config(const char *output_file, const char *root_dir, const char *root_process, char **directories, int dir_count, FileCopy *file_copies, int file_copy_count, Symlink *symlinks, int symlink_count);
