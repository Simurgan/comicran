...
// Function to extract directories from a given path and add them to the directories list
void extract_and_add_directories(char directories[MAX_DIRECTORIES][PATH_MAX], int *dir_count, const char *path) {
    ...
    if (last_slash != NULL && last_slash != temp_path) {
        *last_slash = '\0'; // Terminate to get the directory path
        add_unique_directory(directories, dir_count, temp_path);
    }
}
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < file_copy_count; i++) {
        extract_and_add_directories(directories, &dir_count, file_copies[i].dst);
    }

    // Extract directories from symlinks' sym paths
    for (int i = 0; i < symlink_count; i++) {
        extract_and_add_directories(directories, &dir_count, symlinks[i].sym);
    }
